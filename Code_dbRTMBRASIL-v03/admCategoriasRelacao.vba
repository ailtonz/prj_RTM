            ...
            For intDestino = 0 To ctlDestino.ListCount - 1
                If ctlDestino.Selected(intDestino) Then
                    strSQL = "INSERT INTO admCategorias ( codCategoria, codRelacao,Categoria,Descricao01,Descricao02 ) SELECT " & NovoCodigo("Select * from admCategorias", "codCategoria") & " as Categoria, " & _
                              ctlDestino.Column(0, intDestino) & " as Relacao,  codCategoria, '" & ctlOrigem.Column(1, intOrigem) & "' as Descricao1 ,'" & ctlDestino.Column(1, intDestino) & "' as Descricao2 FROM admCategorias where codCategoria = " & ctlOrigem.Column(0, intOrigem) & ""
                    ExecutarSQL strSQL
...
Dim ctlPrincipal As ListBox
    ...
    strSQL02 = "SELECT admCategorias.codCategoria, admCategorias.Categoria, admCategorias.Descricao01, admCategorias.Descricao02 FROM admCategorias"
    strSQL = "SELECT codCategoria, Descricao01 as Categoria,Descricao02 as SubCategoria FROM (" & strSQL02 & ") as tmp WHERE (((tmp.Categoria)='" & ctlPrincipal.Column(0) & "')) order by codCategoria Desc; "
